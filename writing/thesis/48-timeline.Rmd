...
```{r, echo = FALSE, fig.cap='Visual representation of the estimated timeline for my PhD project.', fig.width=14, fig.height=8, message=FALSE, out.width='100%'}
...
tasks <-
  readr::read_csv('~/GitHub/hr-environ-stoch-phd/writing/thesis/timeline.csv',
                  col_types = 'ccDDl') %>%
  mutate(header = purrr::map2_lgl(section, task, grepl),
         color = header | grepl('publi', tolower(task)),
  ...
  mutate(dec_start = lubridate::decimal_date(start),
         ...
         # to avoid issues with special characters and non-bold numbers
         task = paste0('\'', task, '\''),
         # make headers bold
         task = if_else(bold, paste0('bold(', task, ')'), task))

ggplot(tasks, aes(y = id)) +
  geom_vline(xintercept = decimal_date(as.Date(paste0(2021:2025, '-09-01'))),
             alpha = 0.15, lwd = 0.25) +
  geom_hline(aes(yintercept = id + 0.5), filter(tasks, header)[-7, ], alpha = 0.5) +
  geom_hline(aes(yintercept = id - 0.5), tasks, alpha = 0.05) +
  ...
  geom_text(aes(2020, label = task, color = color), tasks,
            hjust = 'left', size = 3, parse = TRUE) +
  annotate(geom = 'point', x = 2026.1, y = 1, color = 'transparent') +
